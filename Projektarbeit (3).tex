...
Wird der \textit{\glqq Stop!\grqq} -Button betätigt, so öffnet sich ein neues Fenster. In diesem wird abgebildet, dass das Programm zur Laufzeit abgebrochen, welche Ausnahme (\textit{Exception}) geworfen und welche Komponente zu diesem Zeitpunkt ausgeführt wurde (siehe Abb. 4.7).\\
...
\item Ausnahmebehandlung: JAVA unterstützt das sogenannte \textit{exception handling}. Dies ermöglicht es Entwicklern, bei Ausnahmefällen einen Programmsturz zu verhindern und das Programm nach der Fehlerbehandlung weiter arbeiten zu lassen, was sich positiv auf die Verfügbarkeit auswirkt.
